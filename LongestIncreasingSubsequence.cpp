    ...
    int lengthOfLIS(vector<int>& nums) {
        ...
        for(int i = 1; i < n; i++) {
            if(nums[i] > result[length-1]) {
                ...
                auto ind = lower_bound(result.begin(), result.end(), nums[i]); 
                *ind = nums[i];
            }
        }
        return length;
...
//     int util(int curr, int prev, int n, vector<int> nums) {
//         if(curr == n) return 0;
//         int length = util(curr+1, prev, n, nums);
//         if(prev == -1 || nums[curr] > nums[prev]) {
//             length = max(length, 1 + util(curr+1, curr, n, nums));
//         }
//         return length;
//     }
// };
...
//     int util(int curr, int prev, int n, vector<int> nums, vector<vector<int>> &dp) {
//         if(curr == n) return 0;
//         if(dp[curr][prev+1] != -1) return dp[curr][prev+1];
//         int length = util(curr+1, prev, n, nums, dp);
//         if(prev == -1 || nums[curr] > nums[prev]) {
//             length = max(length, 1 + util(curr+1, curr, n, nums, dp));
//         }
